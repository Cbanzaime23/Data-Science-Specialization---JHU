...
library(grid)
```
## Let's load the look the summary of the Activity Data
```{r, echo = TRUE}
...
ggplot(data=Activity_SumNoSteps, aes(x=Number_Steps)) +
...
ggplot(data = Activity_MeanNoSteps, aes(x = interval, y = Mean_NumberSteps)) +
...
ggplot(data=Activity_SumNoSteps_ImputeNAs, aes(x=Number_Steps)) +
    geom_histogram(bins = 20) + 
    geom_vline(aes(xintercept = Mean_NoSteps),col='red',size=2) + 
    geom_vline(aes(xintercept = Median_NoSteps),col='blue',size=2) +
    ggtitle( paste("Mean No. of Steps(RED) =", Mean_NoSteps,'\n',"Median No. of Steps(BLUE) =", Median_NoSteps ) )
...
grid.arrange(WEEKEND_plot, WEEKDAY_plot,ncol = 1, nrow = 2, heights = c(4, 4))
```
